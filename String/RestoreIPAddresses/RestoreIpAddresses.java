...
public class RestoreIpAddresses {
    ...
    private static ArrayList<String> getIP(String s, String currIP, int currIndex, ArrayList<String> res)
    {
	if(currIndex == 3)
            if(s.length() > 0)
	...
	for(int i=1;i<4;i++)
       	   if(s.length()>=i && s.substring(0,i).replaceFirst("^0+(?!$)", "").equals(s.substring(0,i)) && Long.parseLong(s.substring(0,i))<=255)
	       getIP(s.substring(i),currIP+s.substring(0,i)+".",currIndex+1,res);
        return res;
    }

    public static void main(String args[]){
        //String s = "459022042";//"0279245587303"; //"25525511135";
...
/*
It is DFS, using recursive. s.replaceFirst("^0+(?!$)", "") just for remove the leading 0, because  "0X" or "00X" is not valid.
*/
